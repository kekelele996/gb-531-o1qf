...
export const useCoverageEvaluationStore = defineStore('coverage-evaluations', () => {
  ...
  async function run(scenarioId: number, key: string, failureWindowDays?: number) { const item = await coverageApi.runCoverageEvaluation({ scenario_id: scenarioId, failure_window_days: failureWindowDays }, key); await load(); selectedId.value = item.id; return item }
  ...
  async function compare(baseId: number, otherId: number) { return coverageApi.compareCoverageEvaluations(baseId, otherId) }
  return { items, selectedId, selected, loading, load, run, refresh, confirm, voidRun, replay, compare }
})
