...
export const useCoverageEvaluationStore = defineStore('coverage-evaluations', () => {
  ...
  async function run(input: CoverageRunInput, key: string) { const item = await coverageApi.runCoverageEvaluation(input, key); await load(); selectedId.value = item.id; return item }
  ...
  async function compare(id: number, otherId: number): Promise<EvaluationComparison> { return coverageApi.compareCoverageEvaluations(id, otherId) }
  ...
  return { items, selectedId, selected, loading, load, run, refresh, compare, confirm, voidRun, replay }
})
