...
- 覆盖推演：冻结输入，构建原因到后果路径，找出未保护路径，按独立性键去重并保存评分步骤、输入哈希与算法版本；每次评估同时预测未来失效窗口（1–365 天，默认 30 天），按保护层逐个到期日重算覆盖分，标出最早风险升级日期、缺口路径与分数，窗口内无升级时明确显示“无”。
...
6. 未来失效窗口：运行时可填写 `failure_window_days`（1–365，默认 30）。算法收集冻结时刻仍有效保护层的全部验证到期日，按日期升序在每个到期日（含当日）重新执行独立性去重与评分，记录该日覆盖分、残余风险等级、到期保护层和未覆盖缺口路径；第一个使残余风险等级高于基线的日期写入 `first_escalation`，窗口内无任何等级上升时 `escalation_within_window=false` 且前端明确显示“窗口内无风险等级上升”。
7. 窗口参数是冻结输入的一部分：它写入输入快照并参与输入哈希；幂等记录按 `Idempotency-Key + failure_window_days` 联合唯一，同键不同窗口产生独立评估。历史详情、确定性重放（同时校验快照哈希与预测 JSON）和同场景版本对比（比较 `escalation_*_days`）都从冻结快照还原同一预测结果，不随后续保护层变化漂移。

算法不会读取实时仪表、联锁组态、阀位、报警、人员位置或设备通信，也不验证真实独立保护层是否满足法规。未来失效窗口只是基于冻结台账的离线推演，不代表真实到期一定发生。输出必须由持证工艺安全人员结合现场证据复核。
...
运行接口要求 `Idempotency-Key`。同一调用者复用相同键且相同失效窗口时返回已有评估，不重复插入结果；相同键搭配不同 `failure_window_days` 会产生独立评估。历史快照禁止覆盖。
...
- 评估运行返回 `400`：确认请求含非空 `Idempotency-Key`，`failure_window_days` 在 1–365 之间（省略或传 0 使用默认 30 天），且目标场景和保护层数据完整。
- 覆盖分低于预期：检查过期保护层、失效生命周期和重复 `independence_key`，再打开评分解释查看去重原因。
- 未来失效窗口显示与预期不符：窗口只推演冻结时仍有效保护层的“验证到期日”，已过期或已失效保护层不产生事件；事件日期与天数都基于冻结的参考时间，回看历史评估时不会随当前日期漂移。
