...
- 覆盖推演：冻结输入，构建原因到后果路径，找出未保护路径，按独立性键去重并保存评分步骤、输入哈希与算法版本；按未来失效窗口（1–365 天，默认 30 天）逐个保护层到期日重算覆盖分，标出最早风险等级上升日期、缺口路径与分数，窗口内不升级时明确显示“无”，窗口预测随快照与幂等键冻结。
...
运行评估时填写 `failure_window_days`（整数 1–365 天，缺省 0 按默认 30 天处理；越界返回 `400`）：

1. 窗口天数写入冻结快照，参与输入哈希；同一调用者复用相同 `Idempotency-Key` 时连窗口预测一起返回原结果，不会按当前保护层数据重算。
2. 以冻结参考时间为起点，只枚举该时点仍有效保护层的验证到期日 `last_verified_at + test_interval_days`；到期日晚于或等于窗口末日的事件不纳入窗口。
3. 按到期日时间顺序逐个把保护层移出有效集合，重跑同一确定性评分，并记录该日覆盖分、残余风险等级、到期保护层 ID 和未覆盖路径。
4. 与冻结点残余风险等级比较，标出**最早**让风险等级上升的到期日（`earliest_escalation`：日期、距冻结点天数、到期保护层、分数、等级前后、缺口路径）；窗口内不升级时 `no_escalation=true` 且 `earliest_escalation` 为空，页面明确显示“窗口内风险等级不上升：无”。
5. 预测结果随评估持久化（`failure_projection` 列）：历史回看（`GET /:id`）和同场景版本对比（`GET /:id/compare/:other_id` 返回双方窗口摘要与 `failure_projection_changed`）都只还原冻结时的同一预测；确定性重放（`/replay`）也校验存储的投影与快照重算结果一致。
...
| `GET/POST` | `/api/v1/coverage-evaluations` | 评估列表与幂等运行；POST 体含 `scenario_id` 与可选 `failure_window_days`（1–365，默认 30） |
